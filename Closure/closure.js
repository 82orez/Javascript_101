...
// ! 이 함수는 지역 변수를 사용하여 의도치 않은 상태 변경은 방지했지만, 함수를 호출할 때마다 변수가 초기화 되어 결과값은 항상 1이다.
// 즉, 이전 상태를 유지하지 못한다.
count();
count();
...
// 2. 즉시실행함수는 호출된 이후 소멸하지만, 즉시실행함수가 반환한 클로저(inner 함수)는 자신이 정의된 위치에 의해 결정된 상위 스코프인 익명 함수의 렉시컬 환경을 기억하고 있다. (자유 변수 num)
// --> 함수의 실행이 종료되어 해당 함수의 실행 컨텍스트가 소멸되었다 하더라도 그 함수의 렉시컬 환경이 바로 소멸되지는 않는다. (렉시컬 환경은 독립된 객체임)
// --> 만약 그 함수의 렉시컬 환경을 누군가 참조하고 있다면 해당 함수의 렉시컬 환경은 소멸하지 않음. (실행 컨텍스트 참조)
// 3. 따라서 countClosure 변수에 저장된 클로저(inner 함수)는 카운트 횟수의 상태 관리를 위한 자유 변수 num 에 대해 언제, 어디서나 접근해서 참조하거나 변경할 수 있다.
